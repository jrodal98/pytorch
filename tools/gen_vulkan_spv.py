...
def extract_filename(path, keep_ext=True):
    if keep_ext:
...
# https://github.com/google/XNNPACK/blob/master/tools/xngen.py
def preprocess(input_text, variables, input_path="codegen"):
...
class SPVGenerator:
    ...
    def generateVariantCombinations(self, iterated_params, exclude_params={}):
        all_iterated_params = []
        for param_name, value_list in iterated_params.items():
            if param_name not in exclude_params:
                param_values = []
                for value in value_list:
                    param_values.append((param_name, value["SUFFIX"], value["VALUE"]))
                all_iterated_params.append(param_values)

        return list(product(*all_iterated_params))

    def parseTemplateYaml(self, yaml_file):
        yaml_template_params = OrderedDict()
    ...
    def constructOutputMap(self):
        ...
        for shader_name, source_glsl in self.glsl_src_files.items():
            if shader_name not in self.shader_template_params:
                shader_params = copy.deepcopy(self.env)
                self.output_shader_map[shader_name] = (source_glsl, shader_params)

    def generateSPV(self, output_dir):
        output_file_map = {}
...
def gen_cpp_files(spv_files, cpp_header_path, cpp_src_file_path) -> None:
    h = "#pragma once\n"
